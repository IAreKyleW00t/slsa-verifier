...
describe('utils', () => {
  ...
  describe('checkFileSHA256', () => {
    test('accepts a proper hash', async () => {
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'slsa-verifier-tests_')
      )
      const file = path.join(tmpDir, 'testfile.txt')
      fs.writeFileSync(file, 'foobar')
      const expected =
        'c3ab8ff13720e8ad9047dd39466b3c8974e592c2fa383d4a3960714caef0c4f2'
      const result = utils.checkFileSHA256(file, expected)
      expect(result).toBeTruthy()
      fs.rmSync(tmpDir, { recursive: true, force: true })
    })

    test('reject an improper hash', async () => {
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'slsa-verifier-tests_')
      )
      const file = path.join(tmpDir, 'testfile2.txt')
      fs.writeFileSync(file, 'foobar')
      const expected = 'somethingbad'
      const result = utils.checkFileSHA256(file, expected)
      expect(result).toBeFalsy()
      fs.rmSync(tmpDir, { recursive: true, force: true })
    })
    ...
    test('errors when file does not exist', async () => {
      const file =
        process.platform === 'win32'
          ? 'D:\\some\\invalid\\file'
          : '/some/invalid/file'
      const result = (): boolean => utils.checkFileSHA256(file, 'doesntmatter')
